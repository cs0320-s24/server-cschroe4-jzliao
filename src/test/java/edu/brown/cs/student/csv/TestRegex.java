...
/** Credit to: tnelson File taken from assignment's examples */
...
public class TestRegex {
  ...
  /** What if nothing is in between two commas */
  @Test
  public void testEmpty() {
    String line = "Ron,,Harry";
    String[] result = regexSplitCSVRow.split(line);

    assertEquals(3, result.length);
    assertEquals("Ron", result[0].trim().replaceAll("\"", ""));
    assertEquals("", result[1].trim().replaceAll("\"", ""));
    assertEquals("Harry", result[2].trim().replaceAll("\"", ""));
  }

  /** Test when there are spaces in the CSV */
  @Test
  public void testSpaces() {
    String line = "Ron, Hermione,Harry";
    String[] result = regexSplitCSVRow.split(line);

    assertEquals(3, result.length);
    assertEquals("Ron", result[0].trim().replaceAll("\"", ""));
    assertEquals("Hermione", result[1].trim().replaceAll("\"", ""));
    assertEquals("Harry", result[2].trim().replaceAll("\"", ""));
  }

  /** Tests with $ and . in numbers */
  @Test
  public void testDollarSignsAndDecimals() {
    String line = "500, $100.00, 200";
    String[] result = regexSplitCSVRow.split(line);

    assertEquals(3, result.length);
    assertEquals("500", result[0].trim().replaceAll("\"", ""));
    assertEquals("$100.00", result[1].trim().replaceAll("\"", ""));
    assertEquals("200", result[2].trim().replaceAll("\"", ""));
  }
